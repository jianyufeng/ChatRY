...
public class RoundNumber extends View {
    ...
    private boolean drag = true; //设置是否可以拖动
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        if (firstInit) {
            ...
            textSize = radius * 3 / 4; // 根据圆半径来设置字体大小
            ...
            switch (event.getAction()) {
                ...
                case MotionEvent.ACTION_MOVE:
                    if (mClickListener != null) { // 注意这里要用getRaw来获取手指当前所处的相对整个屏幕的坐标
                        mClickListener.onMove(event.getRawX(), event.getRawY() - DensityUtil.getTopBarHeight((Activity) mContext));
//                    mClickListener.onMove(event.get RawX(), event.getRawY());
                    }
                    break;
