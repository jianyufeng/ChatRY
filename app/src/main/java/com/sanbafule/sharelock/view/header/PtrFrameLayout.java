...
public class PtrFrameLayout extends ViewGroup {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (DEBUG && DEBUG_LAYOUT) {
            LogUtil.d(LOG_TAG, String.format("onMeasure frame: width: %s, height: %s, padding: %s %s %s %s", getMeasuredHeight(), getMeasuredWidth(),
        ...
        if (mContent != null) {
            ...
            if (DEBUG && DEBUG_LAYOUT) {
                ...
                LogUtil.d(LOG_TAG, String.format("onMeasure content, width: %s, height: %s, margin: %s %s %s %s",
                        getMeasuredWidth(), getMeasuredHeight(),
                        lp.leftMargin, lp.topMargin, lp.rightMargin, lp.bottomMargin));
                LogUtil.d(LOG_TAG, String.format("onMeasure, currentPos: %s, lastPos: %s, top: %s", mPtrIndicator.getCurrentPosY(), mPtrIndicator.getLastPosY(), mContent.getTop()));
            }
        }
    }
    ...
    private void layoutChildren() {
        ...
        if (mHeaderView != null) {
            ...
            if (DEBUG && DEBUG_LAYOUT) {
                LogUtil.d(LOG_TAG, String.format("onLayout header: %s %s %s %s", left, top, right, bottom));
            }
        }
        if (mContent != null) {
            if (isPinContent()) {
            ...
            if (DEBUG && DEBUG_LAYOUT) {
                LogUtil.d(LOG_TAG, String.format("onLayout content: %s %s %s %s", left, top, right, bottom));
            }
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (DEBUG) {
                    ...
                    LogUtil.v(LOG_TAG, String.format("ACTION_MOVE: offsetY:%s, currentPos: %s, moveUp: %s, canMoveUp: %s, moveDown: %s: canMoveDown: %s", offsetY, mPtrIndicator.getCurrentPosY(), moveUp, canMoveUp, moveDown, canMoveDown));
                }
    ...
    private void updatePos(int change) {
        ...
        if ((mPtrIndicator.hasJustLeftStartPosition() && mStatus == PTR_STATUS_INIT) ||
            ...
            if (DEBUG) {
                LogUtil.i(LOG_TAG, String.format("PtrUIHandler: onUIRefreshPrepare, mFlag %s", mFlag));
            }
        }
        ...
        if (DEBUG) {
            LogUtil.v(LOG_TAG, String.format("updatePos: change: %s, current: %s last: %s, top: %s, headerHeight: %s", change, mPtrIndicator.getCurrentPosY(), mPtrIndicator.getLastPosY(), mContent.getTop(), mHeaderHeight));
        }
    ...
    final public void refreshComplete() {
        ...
        if (delay <= 0) {
            ...
            if (DEBUG) {
                LogUtil.d(LOG_TAG, String.format("performRefreshComplete after delay: %s", delay));
            }
        }
    }
    ...
    private void performRefreshComplete() {
        ...
        if (mScrollChecker.mIsRunning && isAutoRefresh()) {
            // do nothing
            if (DEBUG) {
                LogUtil.d(LOG_TAG, String.format("performRefreshComplete do nothing, scrolling: %s, auto refresh: %s", mScrollChecker.mIsRunning, mFlag));
            }
            return;
        }
    ...
    public void setOffsetToRefresh(int offset) {
        ...
        //mPtrIndicator.setRatioOfHeaderHeightToRefresh(offset/mPtrIndicator.getHeaderHeight()*1f);	//by dzn
    }
    ...
    class ScrollChecker implements Runnable {
        ...
        public void run() {
            ...
            if (DEBUG) {
                if (deltaY != 0) {
                    LogUtil.v(LOG_TAG, String.format("scroll: %s, start: %s, to: %s, currentPos: %s, current :%s, last: %s, delta: %s", finish, mStart, mTo, mPtrIndicator.getCurrentPosY(), curY, mLastFlingY, deltaY));
                }
            }
            if (!finish) {
                mLastFlingY = curY;
        ...
        private void finish() {
            if (DEBUG) {
                LogUtil.v(LOG_TAG, String.format("finish, currentPos:%s", mPtrIndicator.getCurrentPosY()));
            }
            reset();
            onPtrScrollFinish();
        }
        ...
        public void tryToScrollTo(int to, int duration) {
            ...
            if (DEBUG) {
                LogUtil.d(LOG_TAG, String.format("tryToScrollTo: start: %s, distance:%s, to:%s", mStart, distance, to));
            }
            removeCallbacks(this);
