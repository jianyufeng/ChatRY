...
package com.sanbafule.sharelock.zxing.view;
...
import com.sanbafule.sharelock.zxing.CaptureActivity;
...
public final class ViewfinderView extends View {
	...
	public void onDraw(Canvas canvas) {
		...
		if (resultBitmap != null) {
			...
			lineRect.bottom = slideTop + MIDDLE_LINE_WIDTH;// 扫描线的宽度6
			canvas.drawBitmap(((BitmapDrawable) (getResources()
							.getDrawable(R.drawable.qrcode_scan_line))).getBitmap(),
					null, lineRect, paint);
			paint.setColor(getResources().getColor(R.color.android_ciyao_text_color));
			paint.setTextSize(13 * density);
			paint.setAlpha(0xee);
			paint.setTypeface(Typeface.create("System", Typeface.BOLD));
			if ((CaptureActivity.currentState != null)
					&& (CaptureActivity.currentState.equals("onecode"))) {
				float strwidth = paint.measureText(getResources().getString(R.string.scan_onecode));
				float strwidthP = paint.measureText(getResources().getString(R.string.scan_onecode_p));
				canvas.drawText(
						getResources().getString(R.string.scan_onecode),
						(DensityUtil.getDisplayWidth(SApplication.getInstance())-strwidth)/2, (float) (frame.bottom + (float) 20
								* density), paint);
				canvas.drawText(
						getResources().getString(R.string.scan_onecode_p),
						(DensityUtil.getDisplayWidth(SApplication.getInstance())-strwidthP)/2, (float) (frame.bottom + (float) 40
								* density), paint);

			}
			if ((CaptureActivity.currentState != null)
					&& (CaptureActivity.currentState.equals("qrcode"))) {
				float strwidth = paint.measureText(getResources().getString(R.string.scan_qrcode));
				float strwidthF = paint.measureText(getResources().getString(R.string.scan_qrcode_f));
				canvas.drawText(getResources().getString(R.string.scan_qrcode),
						(DensityUtil.getDisplayWidth(SApplication.getInstance())-strwidth)/2, (float) (frame.bottom + (float) 20
								* density), paint);
				canvas.drawText(getResources().getString(R.string.scan_qrcode_f),
						(DensityUtil.getDisplayWidth(SApplication.getInstance())-strwidthF)/2, (float) (frame.bottom + (float) 40
								* density), paint);
			}
